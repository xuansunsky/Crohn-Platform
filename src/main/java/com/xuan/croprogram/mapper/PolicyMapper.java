...
public interface PolicyMapper {
    ...
    @Update("UPDATE medical_policies SET likes = likes + 1 WHERE id = #{id}")
    void incrementLikes(Long id);

    // 🔥 插入：只存 user_id，不存冗余的 nickname 和 avatar
    @Insert("INSERT INTO medical_policies(city_name, policy_type, user_id, update_time, " +
    ...
    @Update("UPDATE medical_policies SET update_time=#{updateTime}, " +
    ...
    @Select("SELECT count(*) FROM medical_policies WHERE city_name = #{city} AND policy_type = #{type} AND user_id = #{userId}")
