...
public interface MessageMapper {
    ...
    // 1. 【发信】把消息塞进信箱
    @Insert("INSERT INTO friend_messages(sender_id, receiver_id, content, type, is_read, created_at) " +
    ...
    @Select("SELECT * FROM friend_messages WHERE " +
    ...
    @Update("UPDATE friend_messages SET is_read = 1 WHERE sender_id = #{friendId} AND receiver_id = #{myId}")
    ...
    @Select("SELECT 'single' AS type, " +
            ...
            ") hit ON hit.id = m.id " +
            "JOIN account_users u ON u.user_id = IF(m.sender_id = #{myId}, m.receiver_id, m.sender_id) " +
